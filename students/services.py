...
class StudentService:
    ...
    def __map_class_marks_of_student(self, student_class_data):
        ...
        for class_data in student_class_data:
            ...
            if (
                student_classes.get(class_data["class_id"], {}).get("total_marks", 0)
                < total_score
            ):
    ...
    def get_classes_attended(self, student_name):
        ...
        """
        student_class_ids = db["grades"].distinct(
            "class_id", {"student_id": self.student_id}
        )
        student_data = {
    ...
    def get_student_performance(self, student_name):
        ...
        student_classes = self.__map_class_marks_of_student(
    ...
    def get_student_class_details(self, class_id, student_name):
        ...
        student_class_data = db["grades"].find(
            filter={"student_id": self.student_id, "class_id": class_id}
        )
        if student_class_data.count() == 0:
            raise ParseError(
                detail="No detail found for this student and it's corresponding class",
                code="not_found",
            )
        student_classes = self.__map_class_marks_of_student(
            student_class_data=list(student_class_data)
