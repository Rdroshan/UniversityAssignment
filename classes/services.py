...
class ClassService:
    ...
    def __map_class_students(self, class_student_data):
        ...
        for student_data in class_student_data:
            ...
            for score in scores:
                ...
                new_score_data.append({"type": score["type"], "marks": score["score"]})
    ...
    def get_class_data(self):
        ...
        student_data = {"class_id": self.class_id, "students": []}
    ...
    def get_class_performance(self):
        ...
        class_data = {"class_id": self.class_id, "students": []}
        # Create mapping between student and their
        student_classes_dict = self.__map_class_students(
    ...
    def get_final_grades(self):
        ...
        student_details_dict = self.__map_class_students(
            class_student_data=class_student_data
        )

        # self.get_graded_student_details(student_details=student_details_dict)
        new_student_details = {}
        sorted_data_on_marks = sorted(
            student_details_dict.items(),
            key=lambda item: item[1]["total_marks"],
            reverse=True,
        )
        rank = 1
        for student_id, details in sorted_data_on_marks:
            grade = self.__get_grade(
                rank=rank, total_students=len(sorted_data_on_marks)
            )
